...
$avatar     = EVENTS_IMG_DIR . htmlspecialchars($event["image"] ?? 'default.jpg', ENT_QUOTES, 'UTF-8');
...
$categorySlug = htmlspecialchars(strtolower($event["category"] ?? 'generica'), ENT_QUOTES, 'UTF-8');

$cardOpacityClass = ($isPast || $isCancelled) ? 'opacity-75' : '';
?>

<article class="card event-card h-100 shadow-sm border-0 rounded-4 overflow-hidden <?php echo $cardOpacityClass; ?>">
    <div class="position-relative">
        <img src="<?php echo $avatar; ?>" alt="Locandina: <?php echo $cleanTitle; ?>" class="card-img-top" style="height: 160px; object-fit: cover;">

        <span class="badge badge-cate-<?php echo $categorySlug; ?> position-absolute top-0 start-0 m-3">
            <?php echo $category; ?>
        </span>

        <?php if (!$isPast && !$isCancelled && ($tabId ?? '') !== 'subscriber-pane'): ?>
            <span class="badge <?php echo htmlspecialchars($statusInfo['class']); ?> position-absolute top-0 end-0 m-3">
        ...
        <?php if ($isCancelled): ?>
            <div class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-danger bg-opacity-25">
                <span class="badge bg-danger shadow-sm">ANNULLATO</span>
            </div>
        <?php elseif ($isPast): ?>
            <div class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-dark bg-opacity-25">
                <span class="badge bg-dark shadow-sm">CONCLUSO</span>
            </div>
        <?php endif; ?>
    </div>

    <div class="card-body d-flex flex-column p-3">
        <h2 class="h6 card-title fw-bold mb-2">
            <?php echo (!empty($cleanTitle)) ? $cleanTitle : 'Evento senza titolo'; ?>
        </h2>
        ...
        <div class="card-text text-muted small mb-3">
            ...
            <div class="mb-1">
                ...
                <span class="visually-hidden">Luogo: </span>
                <?php echo htmlspecialchars($event['location'] ?? 'TBD', ENT_QUOTES, 'UTF-8'); ?>
            </div>
        </div>

        <?php if ((($tabId ?? '') === 'organized-pane' || ($tabId ?? '') === 'draft-pane') && !$isPast && !$isCancelled): ?>
            <div class="mb-3">
                ...
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <?php if (($event['status'] ?? '') === 'DRAFT'): ?>
                            <form action="api/edit_event.php?event_id=<?php echo $event['id']; ?>" method="POST" class="d-inline">
                                <input type="hidden" name="publish_from_draft" value="1">
                                <button type="submit" class="btn btn-primary btn-sm fw-bold shadow-sm">Pubblica</button>
                            </form>
                        <?php endif; ?>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="event.php?event_id=<?php echo $event['id']; ?>" class="btn btn-outline-dark btn-sm px-2" aria-label="Modifica evento: <?php echo $cleanTitle; ?>">
                        ...
                        <button type="button" class="btn btn-outline-danger btn-sm px-2"
                            ...
                            data-event-id="<?php echo $event['id']; ?>"
                            data-action="<?php echo $modalAction; ?>">
                            <span class="bi bi-trash" aria-hidden="true"></span>
                        </button>
                    </div>
                </div>
            <?php else: ?>
                <a href="#?id=<?php echo $event['id']; ?>" class="btn btn-light btn-sm w-100 rounded-3 text-muted border">
